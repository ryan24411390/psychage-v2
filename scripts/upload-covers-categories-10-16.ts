...
/**
 * Upload cover images for article categories 10–16 to Supabase Storage
 * and match article titles to image poster titles.
 *
 * Mapping:
 *   DIG-NNN → CAT10-NNN  (Digital Life, Social Media & Modern Stressors)   50 images
 *   TRA-NNN → CAT11-NNN  (Trauma-Informed Education & Healing)            55 images
 *   CUL-NNN → CAT12-NNN  (Cultural Perspectives, Inclusion & Global)      50 images
 *   MHC-NNN → CAT13-NNN  (Understanding Mental Health Conditions)         80 images (50 articles)
 *   THR-NNN → CAT14-NNN  (Therapy, Treatment & Mental Health Navigation)  50 images
 *   LON-NNN → CAT15-NNN  (Loneliness, Social Connection & Belonging)      46 images (50 articles)
 *   EAT-NNN → SKIPPED    (No matching DB category with articles)          70 images
 *
 * Note: Poster cat 16 (EAT) has eating disorder content but CAT16 in DB is
 *       "Psychosis, Schizophrenia & Severe Mental Illness" — content mismatch.
 *       "Eating & Body" category exists but has 0 articles. EAT is skipped.
 *
 * Usage:
 *   npx tsx scripts/upload-covers-categories-10-16.ts              # Dry-run (default)
 *   npx tsx scripts/upload-covers-categories-10-16.ts --execute     # Upload + rename
 *   npx tsx scripts/upload-covers-categories-10-16.ts --category 10 # Single category
 *   npx tsx scripts/upload-covers-categories-10-16.ts --execute --category 11
 */
...
const CATEGORIES: CategoryConfig[] = [
  {
    ...
    prefix: 'DIG',
    catId: 'CAT10',
    imageDir: '/Users/raiyanabdullah/Downloads/Psychage poster category 10',
  },
  {
    categoryNumber: 11,
    prefix: 'TRA',
    catId: 'CAT11',
    imageDir: '/Users/raiyanabdullah/Downloads/Psychage poster category 11',
  },
  {
    categoryNumber: 12,
    prefix: 'CUL',
    catId: 'CAT12',
    imageDir: '/Users/raiyanabdullah/Downloads/Psychage poster category 12',
  },
  {
    categoryNumber: 13,
    prefix: 'MHC',
    catId: 'CAT13',
    imageDir: '/Users/raiyanabdullah/Downloads/Psychage poster category 13',
  },
  {
    categoryNumber: 14,
    prefix: 'THR',
    catId: 'CAT14',
    imageDir: '/Users/raiyanabdullah/Downloads/Psychage poster category 14',
  },
  {
    categoryNumber: 15,
    prefix: 'LON',
    catId: 'CAT15',
    imageDir: '/Users/raiyanabdullah/Downloads/Psychage poster category 15',
  },
  // EAT (poster cat 16) intentionally excluded — no matching DB articles.
  // CAT16 = Psychosis (wrong topic), "Eating & Body" category has 0 articles.
];
...
async function processCategory(
  ...
  try {
    ...
    return {
      ...
      exactMatches: [], titleMismatches: [], orphanImages: [], articlesWithoutImages: [],
    };
  }
  ...
  if (error || !articles) {
    ...
    return {
      ...
      exactMatches: [], titleMismatches: [], orphanImages: images, articlesWithoutImages: [],
    };
  }
...
function printReport(report: CategoryReport): void {
  ...
  if (titleMismatches.length > 0) {
    ...
    for (const { image, article } of titleMismatches.slice(0, 10)) {
  ...
  if (orphanImages.length > 0) {
    ...
    console.log(`  ${'-'.repeat(86)}`);
    for (const image of orphanImages.slice(0, 10)) {
