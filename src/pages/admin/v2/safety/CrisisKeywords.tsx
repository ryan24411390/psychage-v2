...
const AdminCrisisKeywords: React.FC = () => {
  ...
  const columns: ColumnDef<CrisisKeyword, unknown>[] = [
    { accessorKey: 'keyword', header: 'Keyword / Phrase' },
    {
      ...
      header: 'Lang',
      cell: ({ row }) => <span className="text-xs uppercase text-text-secondary">{row.original.language}</span>,
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <span className="text-sm text-text-secondary">{cat?.label || row.original.category}</span>;
      },
    },
    {
    ...
    },
    {
      ...
      enableSorting: false,
      cell: ({ row }) => (
        <button onClick={() => openDrawer(row.original)} className="p-1.5 text-text-tertiary hover:text-primary" title="Edit">
          <Pencil size={15} />
        </button>
      ),
    },
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <button onClick={() => openDrawer()} className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary-hover text-white text-sm font-medium rounded-lg transition-colors">
      ...
      <div className="mt-6 bg-surface border border-border rounded-2xl p-5">
        <h3 className="text-sm font-semibold text-text-primary mb-3">Test Detection</h3>
        <p className="text-xs text-text-secondary mb-3">Type a sample message to test which keywords would trigger. No safety action is taken.</p>
        <div className="flex gap-2 mb-4">
          <input
            value={testInput}
            onChange={(e) => setTestInput(e.target.value)}
            placeholder="Type a sample message..."
            className="flex-1 px-3 py-2 text-sm border border-border rounded-lg bg-surface outline-none focus:ring-2 focus:ring-primary"
          />
          <button
            onClick={runTestDetection}
            className="px-4 py-2 bg-surface-hover text-text-secondary text-sm rounded-lg hover:bg-surface-active"
          >
            <Search size={16} />
          </button>
        </div>
          ...
          <div className="space-y-2">
            {testResults.map((r) => (
              <div key={r.id} className="flex items-center gap-3 text-sm px-3 py-2 bg-red-50 dark:bg-red-900/20 rounded-lg">
                ...
                <span className="text-text-secondary">({CRISIS_CATEGORIES.find((c) => c.value === r.category)?.label})</span>
              </div>
            ))}
          </div>
        ) : testInput.trim() ? (
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="relative ml-auto w-full max-w-md bg-surface h-full overflow-y-auto shadow-xl p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={closeDrawer} className="text-text-tertiary hover:text-text-secondary"><X size={20} /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">Keyword / Phrase</label>
                <input value={keyword} onChange={(e) => setKeyword(e.target.value)} className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-surface outline-none focus:ring-2 focus:ring-primary" />
              </div>
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">Language</label>
                <select value={language} onChange={(e) => setLanguage(e.target.value)} className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-surface outline-none focus:ring-2 focus:ring-primary">
              ...
              <div>
                ...
                <select value={severity} onChange={(e) => setSeverity(e.target.value as CrisisSeverity)} className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-surface outline-none focus:ring-2 focus:ring-primary">
                  {SEVERITY_LEVELS.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">Category</label>
                <select value={category} onChange={(e) => setCategory(e.target.value as CrisisCategory)} className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-surface outline-none focus:ring-2 focus:ring-primary">
              ...
              <button
                ...
                className="w-full py-2.5 bg-primary hover:bg-primary-hover text-white text-sm font-medium rounded-lg disabled:opacity-50"
              >
