...
import { LogOut, Heart, Crown, Star } from 'lucide-react';
import { cn } from '@/lib/utils';
import InteractiveCard from '@/components/ui/InteractiveCard';
import Badge from '@/components/ui/Badge';
import { useAuth } from '@/context/AuthContext';
import { motion } from 'framer-motion';
import { portalSidebarSections, portalMobileTabItems } from '@/config/sidebars';
import { getTierLabel, getTierBadgeVariant } from '@/lib/provider-tier';
...
const PortalNavSidebar: React.FC<PortalNavSidebarProps> = ({
  providerName,
  tier = 'free',
}) => {
  ...
  const TierIcon = tier === 'elite' ? Crown : tier === 'pro' ? Star : null;
        ...
        <InteractiveCard className="p-4 h-full bg-surface/50 backdrop-blur-md border-white/5 flex flex-col">
          ...
          {providerName && (
            <div className="px-4 pb-4 mb-2 border-b border-gray-200/60 dark:border-neutral-700/50">
              ...
              <Badge variant={getTierBadgeVariant(tier)} className="mt-1.5">
                {TierIcon && <TierIcon size={10} className="mr-1" />}
                {getTierLabel(tier)}
              </Badge>
            </div>
          )}
...
export default PortalNavSidebar;
