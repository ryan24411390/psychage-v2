    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Activities today</label>
          <button
            type="button"
        ...
        {activities.length === 0 && !showAdd && (
          <p className="text-sm text-slate-400 dark:text-slate-500 italic">No activities logged yet</p>
        )}

        {activities.map((activity, i) => (
          <div key={i} className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 mb-2">
        ...
        {showAdd && (
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 space-y-4">
            <input
              ...
              className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
              autoFocus
            />
      ...
      {/* Social interactions */}
      <div>
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Social interactions today</label>
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => setSocialInteractions(Math.max(0, socialInteractions - 1))} className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 flex items-center justify-center text-lg font-medium hover:bg-slate-200">−</button>
          <span className="text-lg font-semibold text-slate-900 dark:text-white w-8 text-center">{socialInteractions}</span>
          <button type="button" onClick={() => setSocialInteractions(socialInteractions + 1)} className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 flex items-center justify-center text-lg font-medium hover:bg-slate-200">+</button>
        </div>
      </div>
