  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-24 pb-12 px-6">
      ...
      <div className="container mx-auto max-w-2xl">
        ...
        <h1 className="font-display font-bold text-2xl md:text-3xl text-slate-900 mb-2">Wellness Toolbox & Safety Plan</h1>
        ...
        {activeTab === 'toolbox' ? (
                        ...
                        <div className="px-5 pb-5 space-y-2">
                          {items.map(item => (
                            <div key={item.id} className="flex items-center gap-3 bg-slate-50 rounded-xl p-3">
                              ...
                              <button onClick={() => deleteToolboxItem(item.id)} className="text-slate-300 hover:text-red-500 transition-colors" aria-label="Delete">
                          ...
                          {addingToCat === cat.id ? (
                            <div className="flex gap-2">
                              <input value={newItemText} onChange={e => setNewItemText(e.target.value)} placeholder="Add an item..." className="flex-1 px-3 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" autoFocus />
          ...
          <div className="space-y-4">
            ...
            {SAFETY_PLAN_SECTIONS.map(section => {
              ...
              return (
                <div key={section.number} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                  ...
                  <div className="mt-3 space-y-2">
                    {items.map(item => (
                      <div key={item.id} className="flex items-center gap-3 bg-slate-50 rounded-xl p-3">
                        ...
                        <button onClick={() => handleDeleteSafetyItem(section.number, item.id)} className="text-slate-300 hover:text-red-500 transition-colors" aria-label="Delete">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    ))}
                    ...
                    {addingToSection === section.number ? (
                      <div className="space-y-2 bg-slate-50 rounded-xl p-3">
                        <input value={spText} onChange={e => setSpText(e.target.value)} placeholder={section.placeholder} className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" autoFocus />
                        {'hasContact' in section && section.hasContact && (
                          <div className="grid grid-cols-2 gap-2">
                            <input value={spName} onChange={e => setSpName(e.target.value)} placeholder="Name (optional)" className="px-3 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
                            <input value={spPhone} onChange={e => setSpPhone(e.target.value)} placeholder="Phone (optional)" className="px-3 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
                          </div>
                        )}
                        <div className="flex gap-2">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-900 text-sm font-medium transition-colors"
            >
