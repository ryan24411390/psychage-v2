...
const WeeklyReflection: React.FC = () => {
  ...
  const textAreaClass = "w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none min-h-[80px]";
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-24 pb-12 px-6">
      ...
      <div className="container mx-auto max-w-2xl">
        ...
        <h1 className="font-display font-bold text-2xl md:text-3xl text-slate-900 mb-1">Weekly Reflection</h1>
        <p className="text-slate-500 mb-2">Step back. See the bigger picture.</p>
        <p className="text-sm text-slate-400 mb-6">{formatDateRange(weekStart, weekEnd)}</p>

        {/* Week summary */}
        <WeekSummaryMini checkIns={weekCheckIns} />

        {saved ? (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100 text-center">
            <div className="w-14 h-14 rounded-full bg-teal-100 text-teal-600 flex items-center justify-center mx-auto mb-4">
              <Check className="w-7 h-7" />
            </div>
            <h2 className="font-display font-semibold text-xl text-slate-900 mb-2">Reflection saved.</h2>
            <p className="text-slate-500 mb-6">Nice work looking back on your week.</p>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-100 space-y-6">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">What went well this week?</label>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">What was difficult this week?</label>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Did you notice any patterns in your mood, energy, or sleep?</label>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Is there one thing you want to do differently next week?</label>
              <textarea value={doNext} onChange={e => setDoNext(e.target.value)} placeholder="One small change." className={textAreaClass} />
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">What is one thing you are grateful for, or one moment of clarity you experienced?</label>
              <textarea value={gratitude} onChange={e => setGratitude(e.target.value)} placeholder="" className={textAreaClass} />
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Clarity Score (optional)</label>
              <input
                type="number"
                min={0}
                max={100}
                ...
                className="w-full sm:w-48 px-4 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
              />
            </div>
