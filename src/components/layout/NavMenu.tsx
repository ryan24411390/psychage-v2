  ...
  return (
    <div className="w-full" onMouseLeave={onMouseLeave} role="menu">
      <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {menu.sections?.map((section, idx) => (
              <div
                ...
                className="space-y-4"
              >
                <h3 className="text-xs font-semibold text-text-tertiary uppercase tracking-wider">{section.title}</h3>
                    ...
                    return (
                      <div key={item.id}>
                        <Link
                          ...
                          className="group relative flex items-start gap-4 p-3 rounded-2xl hover:bg-surface-hover transition-all duration-200"
                          role="menuitem"
                        >
                          <div className="w-10 h-10 shrink-0 rounded-xl bg-surface shadow-sm border border-border p-2 group-hover:scale-105 transition-transform duration-300 flex items-center justify-center text-teal-600">
                            <Icon size={20} />
                          </div>
                          <div>
                            <h4 className="font-bold text-sm text-text-primary leading-tight group-hover:text-teal-600 transition-colors">
                              {item.label}
                            </h4>
        ...
        <div className="w-full md:w-72 border-l border-border pl-8 flex flex-col justify-between">
          <motion.div
              ...
              {menu.quickActions?.map((action, idx) => (
                <li key={idx}>
                  <Link
                    ...
                    className="flex items-center justify-between py-2 px-3 -mx-3 rounded-lg text-sm font-semibold text-text-secondary hover:text-teal-700 dark:hover:text-teal-400 hover:bg-surface-hover transition-all group"
                    role="menuitem"
                  >
                    <span>{action.label}</span>
                    <ChevronRight size={14} className="text-text-tertiary group-hover:text-teal-500" />
                  </Link>
                </li>
              ))}
            </ul>
