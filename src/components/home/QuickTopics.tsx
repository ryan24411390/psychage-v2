...
const QuickTopics: React.FC = () => {
    ...
    return (
        <section className="py-16 px-6 border-b border-gray-100 bg-white">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                    {topics.map((topic, index) => (
                        <motion.button
                            ...
                            className="flex flex-col items-start text-left p-6 rounded-xl border border-gray-100 hover:border-gray-300 bg-gray-50 hover:bg-white transition-all duration-200 group"
                        >
                            <topic.icon size={24} className="text-gray-400 group-hover:text-gray-900 mb-4 transition-colors" />
                            <h3 className="font-bold text-gray-900 mb-1">{topic.label}</h3>
                            <p className="text-xs text-gray-500 font-medium">{topic.desc}</p>
                        </motion.button>
                    ))}
                </div>
            </div>
