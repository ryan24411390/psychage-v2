...
export default function CrisisOverlay({ isAutoTriggered, onClose }: CrisisOverlayProps) {
  return (
    <motion.div
      ...
      className="fixed inset-0 z-[999] bg-white dark:bg-slate-900 flex items-center justify-center p-6"
    >
      <div className="max-w-md w-full text-center">
        ...
        <div className="space-y-3">
          ...
          <a
            ...
            className="flex items-center gap-3 w-full px-5 py-4 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white rounded-xl transition-colors text-left"
          >
        ...
        <button
          onClick={onClose}
          className="mt-8 inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
        >
          <ArrowLeft size={14} />
          Return to chat
        </button>
