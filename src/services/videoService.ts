...
import { withFallback, queryOneWithFallback } from '../lib/withFallback';
...
export const videoService = {
    getAll: async (params?: { category?: string; limit?: number }): Promise<Video[]> =>
        withFallback(
            async () => {
                ...
                if (params?.category) query = query.ilike('category.name', params.category);
                if (params?.limit) query = query.limit(params.limit);
                const { data, error } = await query;
                if (error) throw error;
                return (data as unknown as DBVideo[] || []).map(mapToVideo);
            },
            () => {
                let result = mockVideos as unknown as Video[];
                if (params?.category) result = result.filter(v => v.category.toLowerCase() === params.category?.toLowerCase());
    ...
    getById: async (id: number | string): Promise<Video | undefined> =>
        queryOneWithFallback(
            () => supabase.from('videos').select('*, category:categories(name)').eq('id', id).single(),
            (row: DBVideo) => mapToVideo(row),
            () => (mockVideos as unknown as Video[]).find(v => v.id.toString() === id.toString()),
    ...
    getPopular: async (limit: number = 5): Promise<Video[]> =>
        withFallback(
            ...
            () => (mockVideos as unknown as Video[]).sort((a, b) => b.views - a.views).slice(0, limit),
            'videoService.getPopular'
        ),
