-- =============================================================================
-- search_providers_v3 — geo radius + state correctness + cheaper count
-- Changes vs. search_providers_v2 (20260411000008):
--   1. Adds p_latitude, p_longitude, p_radius_miles for "Near me" radius search.
--      Bounding-box prefilter (uses idx_provider_locations_geo) + haversine
--      post-filter (no extension required).
--   2. State filter: pl.state_province ILIKE p_state  →  = upper(p_state)
--      (frontend parseLocation always normalizes to 2-letter abbr; ILIKE on
--      2-letter codes was a substring match and leaked unrelated states).
--   3. Total-count cap lowered 100k → 10k for filtered path (UI only paginates;
--      exact totals beyond 10k are not displayed).
--   4. ORDER BY adds distance-asc as the primary key when geo is active.
-- v2 is left in place for rollback; drop in a follow-up migration once stable.
-- =============================================================================

DROP FUNCTION IF EXISTS public.search_providers_v3;

CREATE OR REPLACE FUNCTION public.search_providers_v3(
...
DECLARE
  ...
  v_use_geo boolean := (p_latitude IS NOT NULL AND p_longitude IS NOT NULL AND p_radius_miles IS NOT NULL);
...
BEGIN
  IF v_use_geo THEN
    -- Bounding box: 1° latitude ≈ 69 miles; 1° longitude ≈ 69·cos(lat) miles.
    -- GREATEST guards against div-by-zero near the poles (not a real-world risk for US data).
    v_min_lat := p_latitude - (p_radius_miles / 69.0);
    v_max_lat := p_latitude + (p_radius_miles / 69.0);
    v_min_lng := p_longitude - (p_radius_miles / (69.0 * GREATEST(cos(radians(p_latitude)), 0.01)));
    v_max_lng := p_longitude + (p_radius_miles / (69.0 * GREATEST(cos(radians(p_latitude)), 0.01)));
  END IF;
  ...
  IF v_has_filters THEN
    SELECT LEAST(COUNT(DISTINCT p.id), 10000) INTO v_total
    FROM public.providers p
    LEFT JOIN public.provider_locations pl ON pl.provider_id = p.id AND pl.is_primary = true
    WHERE p.status IN ('active', 'seeded')
      ...
      AND (p_verification_status IS NULL OR p_verification_status = '' OR
        (p_verification_status = 'verified' AND (p.verified_at IS NOT NULL OR p.status IN ('verified', 'active')))
        OR
        (p_verification_status = 'listed' AND p.status = 'seeded' AND p.verified_at IS NULL)
      )
      AND (NOT v_use_geo OR (
        pl.latitude BETWEEN v_min_lat AND v_max_lat
        AND pl.longitude BETWEEN v_min_lng AND v_max_lng
        AND (3959 * 2 * asin(sqrt(
          power(sin(radians((pl.latitude - p_latitude)/2)), 2) +
          cos(radians(p_latitude)) * cos(radians(pl.latitude)) *
          power(sin(radians((pl.longitude - p_longitude)/2)), 2)
        ))) <= p_radius_miles
      ));
  ELSE
    SELECT GREATEST(c.reltuples::bigint, 0) INTO v_total
  ...
  ORDER BY
    ...
    CASE WHEN v_use_geo AND p_sort != 'name' THEN
      3959 * 2 * asin(sqrt(
        power(sin(radians((pl.latitude - p_latitude)/2)), 2) +
        cos(radians(p_latitude)) * cos(radians(pl.latitude)) *
        power(sin(radians((pl.longitude - p_longitude)/2)), 2)
...
GRANT EXECUTE ON FUNCTION public.search_providers_v3(
  text, uuid[], text[], uuid[], uuid[], uuid[],
