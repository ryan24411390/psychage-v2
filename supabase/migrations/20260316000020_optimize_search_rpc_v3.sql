-- =============================================================================
-- Optimize search_providers_v2 for 200k+ scale
--
-- Changes:
--   1. Use estimated count for unfiltered queries (fast reltuples lookup)
--   2. Only compute exact count when filters are applied
--   3. Add LIMIT on count subquery to cap at 10,000 (beyond that, show "10,000+")
--   4. Use partial index hints via status filter
-- =============================================================================
...
BEGIN
  ...
  ELSE
    ...
    -- If stats are stale (0 or negative), do a real count
    IF v_total <= 0 THEN
      SELECT COUNT(*) INTO v_total FROM public.providers WHERE status IN ('active', 'seeded');
    END IF;
  END IF;
