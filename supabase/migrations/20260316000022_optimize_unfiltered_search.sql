-- =============================================================================
-- Optimize unfiltered search for 200k+ providers
--
-- When no filters are applied, sorting 200k+ rows by tier+name is too slow
-- for Supabase's statement timeout. Fix: use a fast index-backed sort for
-- the unfiltered case (sort by id = index scan) and skip the expensive count.
-- =============================================================================
...
BEGIN
  ...
  IF v_has_filters THEN
    -- Exact count for filtered queries (capped at 100k for performance)
  ...
  ELSE
    -- Fast estimated count for unfiltered queries (reads pg_class stats)
    SELECT GREATEST(c.reltuples::bigint, 0) INTO v_total
    FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE c.relname = 'providers' AND n.nspname = 'public';

    -- If stats are stale (0 or negative), use a safe estimate
    IF v_total <= 0 THEN
      v_total := 200000; -- Known approximate count after bulk seed
    END IF;
  END IF;
...
-- Add covering index for the default unfiltered sort order
CREATE INDEX IF NOT EXISTS idx_providers_default_sort
  ON public.providers (tier, display_name)
